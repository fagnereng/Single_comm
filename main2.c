...
int main(void)
{
	...
	GPIOA->ODR |= (1<<7) | (1<<6) | 1;		//inicia com leds e buzzer desligados e saída COM em idle
	...
	while(1)
	{
		...
		if(!(GPIOA->IDR & 1))	//verifica se uma comunicação foi iniciada
		{
...
void envia_cmd(uint8_t dado)
{
	...
	Delay_us(25);				//aguarda para ler o bit de resposta
	if((GPIOA->IDR & 1))		//verifica se houve resposta da outra placa
	{	//se não houve, atua localmente nos LEDs
		if(dado_copia == 0b01)
		{
			GPIOA->ODR ^= 1 << 6;	//alterna o estado do LED em PA6
		}
	...
	buzzer();					//sinaliza o fim da operação
}
...
uint8_t recebe_cmd(void)
{
	...
	if(!(GPIOA->IDR & 1))	//confirma que houve um start bit
	{
		...
		{
			Delay_us(25);		//aguarda para enviar um bit de resposta
			GPIOA->ODR &= ~1;	//bit de resposta
			Delay_us(50);		//tempo do bit
			GPIOA->ODR |= 1;	//libera a linha de comunicação
